...
class FirmwarePipeline(FilesPipeline):
    ...
    def item_completed(self, results, item, info):
        ...
        try:
            ...
            if not product_id:
                cur.execute(
                    "INSERT INTO product (iid, url, mib_filename, mib_url, mib_hash, sdk_filename, sdk_url, sdk_hash, product, version, build, date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                ...
                cur.execute(
                    "UPDATE product SET iid=(?), url=(?), mib_filename=(?), mib_url=(?), mib_hash=(?), sdk_filename=(?), sdk_url=(?), sdk_hash=(?), product=(?), version=(?), build=(?), date=(?) WHERE id=(?)",
                    (image_id, item["url"], status["mib"]["path"], item.get("mib", None), status["mib"]["checksum"],
                     status["sdk"]["path"], item.get("sdk", None), status["sdk"]["checksum"], item.get("product", None),
                     item.get("version", None), item.get("build", None), item.get("date", None), image_id))
